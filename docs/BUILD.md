...
> **⚠️ ルール**: ビルド完了を報告する際は **必ずリリースノートをセットで出すこと**。
> 前回リリースノートを出したコミットから `HEAD` までの差分を `git log <前回commit>..HEAD --oneline` で確認し、
> Play Store 用 (ユーザー向け) と 社内 Changelog (技術メモ) の両方を作成する。
> 今月の残りビルド数も合わせて報告する。
